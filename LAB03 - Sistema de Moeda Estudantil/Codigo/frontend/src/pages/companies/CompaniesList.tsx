...
export default function CompaniesList() {
  ...
  const filtered = items.filter(e =>
    [e.nome, e.email, e.login].join(' ').toLowerCase().includes(q.toLowerCase())
  )
  ...
  return (
    <>
      ...
      {/* 🔥 CONTEÚDO DA PÁGINA */}
      <div className="card max-w-5xl mx-auto mt-10 backdrop-blur-sm bg-white/10 border-white/20">

        <div className="flex items-center justify-between gap-3 mb-4">
          <div>
            <div className="text-xl font-bold text-white">Empresas Parceiras</div>
            <p className="text-white/70">Gerencie cadastros</p>
          </div>
        ...
        {loading ? (
          ...
          <div className="overflow-x-auto">
            ...
            {filtered.length === 0 && (
              <p className="text-white/60 mt-3">Nenhuma empresa encontrada.</p>
            )}
          </div>
        )}
      </div>
